%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Main file for Simulation 3 for Universal Solution to the TIE.
% Version 2.0 -
% Related Reference:
...
Flag_Hann = 1;      % 1: Use Hann distribution phase object.
% 0: Do not use Hann phase object.

Flag_AperShape = 'ellipse';
% 'rect': Rect.
...
sigma = 0.8e-4;    % sigma of Gaussian intensity distribution (m)

% Symbolize expression.
syms Phase_sym Inten_sym x y
Inten_sym = 1-exp((-x.^2-y.^2)/(2*sigma.^2));
...
subplot(2,3,6)
imshow(phiErr_US,[-0.1,0.1]);
colormap(gca,jet);colorbar
title(['Error (US-TIE) ','RMSE = ', num2str(RMSE_US(end))]);

figure
plot(0:size(RMSE_US,2)-1,RMSE_US,'color',[0.2,0.2,0.8],'LineWidth',3);
hold on
plot(0:size(RMSE_IterDCT,2)-1,RMSE_IterDCT,'color',[0.8,0.2,0.2],'LineWidth',3);
